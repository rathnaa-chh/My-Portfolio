...
import siteConfiguration from './.do/make/site.json'
...
function SiteConfiguration(config: SiteConfiguration): Plugin {
  ...
  const title = config.title ?? "Rathna Chhourn — Frontend Developer"
...
function MakeKitPlugin(options: { storiesGlob: string | string[] }): Plugin {
  ...
  const ROUTE = '/.do/make/kit.html'
  const VIRTUAL_ID = 'virtual:Rathna-stories'
  ...
  const HTML_BOOTSTRAP = `<!doctype html>
      <html lang="en">
        ...
        <body>
          ...
          <script type="module">
            import { stories } from 'virtual:Rathna-stories'
            window.__Rathna__ = Object.assign(window.__Rathna__ ?? {}, { stories })
            window.dispatchEvent(new CustomEvent('Rathna.ready'))
          </script>
        </body>
      </html>`
  return {
